...
function projectDirective() {
        ...
        template: (
            '<div class="col-md-4">'+
              '<div class="card mb-4 shadow-sm">'+
                '<img class="card-img-top" src="{{project.photo}}" alt="Card image cap">'+
                '<div class="card-body">'+
                  '<p class="card-text">{{project.text}}</p>'+
                  '<div class="d-flex justify-content-between align-items-center">'+
                    '<div class="btn-group">'+
                      '<a href="{{project.link}}" target="_blank"><button type="button" class="btn btn-sm btn-outline-secondary">View</button></a>'+
                    '</div>'+
                  '</div>'+
                '</div>'+
              '</div>'+
    ...
    function link(scope) {
        if (!scope.project.photo) {
            scope.project.photo = 'https://www.drupal.org/files/issues/default-avatar.png';
        }
    }
